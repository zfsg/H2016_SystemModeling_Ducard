...
\importname{Kinetic energy: translation}{$T_t(t)=\frac{1}{2}\left(v_{x,cg}^2+v_{y,cg}^2\right)$}
...
\importname{Moment of Inertia}{$J=\int_V r_\perp^2 \rho dV$}
...
\note{$R_i=\SI{8.314}{\joule\per\kelvin\per\mole}$, Quantity $n$ in $[\si{\per\mole}]$, Pressure $P$ in $[\si{\pascal}]$, Volume $V$ in $[\si{\meter\cubed}]$, Temperature $T$ in $[\si{\kelvin}]$}
...
\important{$\overset{\ast}{Q}=A\cdot\alpha\cdot(T_1-T_2)$}

\note{$\alpha:$ heat transfer coefficient in $[\si{\watt\per\kelvin\per\meter\squared}]$}

\importname{Newtons law}{$\overset{\ast}{Q}=k\cdot(T_1-T_2)$}
...
$\ddt U_{2,j}$&$m_2c_2\frac{d\vartheta_{2o,j}(t)}{dt}=\overset{\ast}{m}_2c_2(\vartheta_{2i,j}(t)-\vartheta_{2o,j}(t))+\overset{Q}{\ast}_j(t)$
}
...
\begin{enumerate}
\item Input/Output: mass flows $\overset{\ast}{m}_{in/out}(t)$ and enthalpy flows $\overset{H}{\ast}_{in/out}(t)$\\
...
$\Pi_t=\frac{p_3}{p_4}$: Pressure ratio
...
\important{$\overset{\ast}{H}_{flow}(\vartheta)+\overset{\ast}{Q}_{chem}(\vartheta)=0\qquad \overset{\ast}{H}_{flow}=\overset{\ast}{m}c_p(\vartheta_i-\vartheta)$}
