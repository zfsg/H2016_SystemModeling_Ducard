...
$F_{r,x}(t)$&$\frac{1}{2} \cdot \rho \cdot v^2(t) \cdot \frac{Al}{d} \cdot \text{sign}[v(t)] \cdot \lambda(v(t))$}
...
$p(t)$&$\frac{1}{\sigma_0V_0}\Delta V(t)$\\
$\Delta V(t)$&$V(t)-V_0$
}
...
\mportname{Mechanical part}{$m\cdot\ddot{x}(t)=F(t)-k\cdot x(t)-d\cdot \dot{x}(t)$}
...
\mportname{Enthalpy flow}{$\overset{\ast}{H}(t)=\overset{\ast}{m}(t) \cdot C_p(T(t)) \cdot T(t)$}
...
\important{$n=\SI{1}{\mole}=\SI{6.022e23}{}$}

\importname{\note{Rate of formation left to right}}{$\frac{d^+}{dt}[C]=\gamma\cdot r^+\cdot[A]^\alpha\cdot[B]^\beta$}

\importname{\note{Rate of formation right to left}}{$\frac{d^-}{dt}[C]=-\gamma\cdot r^-\cdot[C]^\gamma\cdot[D]^\delta$}

\importname{\note{\\Total rate of formation}}{$\ddt[C]=\gamma\left(r^+\cdot[A]^\alpha\cdot[B]^\beta-r^-\cdot[C]^\gamma\cdot[D]^\delta\right)$}

More C $\rightarrow$ stronger decomposition of C $\rightarrow$ pay attention to sign when negative feedback is happening!
...
\importname{\note{\\Total rate of formation of the backward reaction}}{$\ddt[A]=\alpha\left(r^-\cdot[C]^\gamma\cdot[D]^\delta-r^+\cdot[A]^\alpha\cdot[B]^\beta\right)$}
...
\mportant{$\tilde{e} = \tilde{y} - H \cdot \pi$}
...
\importname{Transfer function}{$P(s)=C\cdot[sI-A]^{-1}\cdot B+D$}
...
$y(t)$&$\begin{bmatrix}\textcolor{orange}{b_0}&\cdots&\textcolor{orange}{b_{n-r-1}}&\textcolor{orange}{1}&0\cdots&0\end{bmatrix}x(t)=\textcolor{orange}{C}x(t)$
}
